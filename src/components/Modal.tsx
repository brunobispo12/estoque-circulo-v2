...
function Modal({ children, setModal, modalTitle }: ModalType) {
    return (
        <div className='z-50 fixed left-0 top-0 flex justify-center items-center h-screen w-full bg-gray-600 bg-opacity-50'>
            <motion.div className='bg-smoke w-[35vw] h-[75vh] rounded border-raisin border-2 p-3 space-y-7 overflow-y-auto scrollbar-thumb-princeton scrollbar-thin scrollbar-track-gray-300'
                initial={{ x: -75, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.2 }} exit={{ x: 75, opacity: 0 }}
            >
